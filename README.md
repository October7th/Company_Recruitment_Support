...
## 4. 보완점 및 업데이트 정보
### ▶ 보완점
 * 회원가입 페이지에서 생년월일, 성별, 이메일, 휴대전화 미입력시 알림창 뜨게 하기
 
   ###### → 현재 아이디, 비밀번호, 비밀번호확인, 이름만 적용되어있음
 * 회원가입 시 아이디 중복 검사 추가하기
 * 회원가입 한 User 정보로 로그인 기능 추가하기
 
   ###### → 현재 로그인 기능 없음
 * 지원서 작성 step1 인적사항에서 증명사진 업로드 후 이미지 보이게 하기
 * 장애여부, 보훈여부 세부내용 수정하기
 
   ###### → 현재 'test'로 작성되어있음
 * step4 자기소개서 작성할 때 글자수 오류 고치기

   ###### → 원래 글자수 나오는데 파일 옮긴 후 1번 문항을 제외하고 글자수가 뜨지않음
 * 지원자 현황 확인이 가능 한 관리자 전용 로그인 만들기
 * 관리자모드로 지원자 목록 봤을 때 파일 다운로드 가능하게 만들기
